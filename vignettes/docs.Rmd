...
Growth standard calculation has been migrated to its own package, [growthstandards](https://github.com/hbgdki/growthstandards), but since it is used so frequently in this package and to help with backward compatibility, all of the functions in growthstandards are re-exported in hbgd.

Utility functions for conveniently converting anthropometric measurements to z-scores or centiles (and converting z-scores / centiles to measurements) for three major growth standards are provided:
...
fit <- fit_trajectory(subset(smc, subjid == 10001), wandfit)
```
...
To apply a fitting method honoring the new holdout variable, we simply set `holdout = TRUE` in our call to `r rdl(get_fit())`.  Let's do this for "wand".

```{r smocc_holdout2, cache=TRUE, warning=FALSE}
wandfit_hold <- get_fit(smc, y_var = "haz", method = "wand", holdout = TRUE)
```

We can now apply the fit to each subject.

```{r smocc_holdout3, cache=TRUE, message=FALSE}
smc_tr_wand_hold <- fit_all_trajectories(smc, wandfit_hold)
```
...
get_fit_holdout_mse(smc_tr_wand_hold)
```

We can apply the same approach to other fitting methods and compare the MSE. Below, we will show how we can examine Trelliscope displays to visually investigate overfitting, etc.
...
One useful feature of Trelliscope is the ability to compare fits across different methods.  For example, suppose we wish to fit the "smooth.spline" method and compare the fitted values visually.
...
trscope_trajectories(smc_tr_ss, z = TRUE)
```
