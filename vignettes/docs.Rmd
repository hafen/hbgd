...
To apply a fitting method honoring the new holdout variable, we simply set `holdout = TRUE` in our call to `r rdl(get_fit())`.  Let's do this for "wand" and "brokenstick".
...
For this case, wand has a better MSE than brokenstick. Below, we can examine Trelliscope displays to visually compare the fits to see if one appears to be overfitting vs. the other.
